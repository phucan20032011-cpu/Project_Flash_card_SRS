...
// Vanilla JS thuần, không dùng thư viện ngoài
// =============================================
...
async function deleteCard(cardId) {
    confirmDelete('Xóa thẻ này?', async () => {
        const result = await fetchAPI('/flashcard/api/delete_card.php', { card_id: cardId });
        if (result.success) {
...
async function deleteSet(setId) {
    confirmDelete('Xóa bộ từ vựng này và toàn bộ thẻ bên trong?', async () => {
        const result = await fetchAPI('/flashcard/api/delete_set.php', { set_id: setId });
        if (result.success) {
...
async function toggleFlag(cardId, btn) {
    const result = await fetchAPI('/flashcard/api/flag_card.php', { card_id: cardId });
    if (result.success) {
